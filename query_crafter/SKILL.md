...
python scripts/query_generator.py --scope scope.json --all
# 每平台 5 个分层检索式（broad/precise/angle_tech/angle_app/review）
...
返回 JSON `{platform: [{"variant","label","query"}, ...]}`，每平台 5 个层次，共同覆盖综述参考文献范围：
...
- Literature time span
- Search focus: `review articles` (for V1) or `all literature types` (for V2)
