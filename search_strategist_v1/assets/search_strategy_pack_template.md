...
7. 双语侧车必须提供完整正文译文，而不是仅翻译标题或个别段落。标题、说明性文字、表头、首列字段标签和受控配置值需要翻译；关键词与排除词列表、平台名、A0/A1/B、检索语法、DOI 和文献元数据不得改写。中英文正文的标题层级、表格形状、列表数量和代码块数量必须一致，不得新增或删除约束。
...
- [ ] 语言：国际库使用英文词表、中文库使用中文词表；范围卡与使用说明同时具备结构完整、无残留大段未翻译说明的 `zh`、`en` 正文；检索式页与候选清单页的结构化界面文字可切换中英文
- [ ] 范围：排除项与 Step 1 一致
- [ ] 互锁：四份文件口径一致
- [ ] 待补项：所有【待补】已向用户确认或说明
