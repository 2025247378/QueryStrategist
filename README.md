...
- **完整流程**：调用根 `SKILL.md`（主 Skill），Step 0 先配置写作类型与目标语言/期刊，Step 1 收敛范围（G1 确认），Step 2 在 Search B 联网前请求一次授权，再执行双通道检索并交付检索策略包（G2 确认）。拒绝授权时只跳过 Search B，Search A 与策略包继续。
