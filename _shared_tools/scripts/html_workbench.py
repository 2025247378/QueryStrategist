...
WRITING_TYPE_I18N = {
    "通用检索": "writing_general",
    "general search": "writing_general",
    "general": "writing_general",
    "综述": "writing_review",
...
function variantKey(text){const value=text.trim();if(/^A0\b/i.test(value))return'variant_a0';if(/^A1\b/i.test(value))return'variant_a1';if(/^B\b/i.test(value))return'variant_b';if(/^C\b/i.test(value))return'variant_c';if(/^D1\b/i.test(value))return'variant_d1';if(/^D2\b/i.test(value))return'variant_d2';if(/^E\b/i.test(value))return'variant_review_oriented';if(/review|survey|综述/i.test(value))return'variant_review';return''}
...
function headerKey(value){const text=value.trim().toLowerCase();if(/^(no\.?|id|序号|编号)$/.test(text))return'header_no';if(/first author|第一作者/.test(text))return'header_first_author';if(/^(title|题名|标题)$/.test(text))return'header_title';if(/author|作者/.test(text))return'header_authors';if(/^(year|年份|年)$/.test(text))return'header_year';if(/journal|source|venue|期刊|来源/.test(text))return'header_source';if(/^doi\b/.test(text))return'header_doi';if(/^oa\b|open access|开放获取/.test(text))return'header_oa';if(/verification|status|验证|状态|核验/.test(text))return'header_status';if(/abstract|摘要/.test(text))return'header_abstract';if(/keyword|关键词/.test(text))return'header_keywords';return''}
function searchFocusKey(value){const text=value.trim().toLowerCase();if(/review|综述/.test(text))return'focus_review';if(/precision|查准/.test(text))return'focus_precision';if(/novelty|新颖/.test(text))return'focus_novelty';return'focus_balanced'}
function metricKey(value){const text=value.trim().toLowerCase();if(/harvested raw|原始收割/.test(text))return'metric_harvested_raw';if(/deduplicated|去重后/.test(text))return'metric_deduplicated';if(/duplicates removed|移除重复/.test(text))return'metric_duplicates_removed';if(/^verified$|^已验证$/.test(text))return'metric_verified';if(/unverified|待人工/.test(text))return'metric_unverified';if(/dropped|已剔除/.test(text))return'metric_dropped';return''}
function markCandidateStatistics(article){Array.from(article.querySelectorAll(':scope>.table-wrap table')).forEach(table=>{const headers=Array.from(table.tHead?.rows[0]?.cells||[]);if(headers.length!==2||!/metric|指标/i.test(headers[0].textContent)||!/value|数值/i.test(headers[1].textContent))return;headers[0].dataset.i18n='metric_label';headers[1].dataset.i18n='value_label';Array.from(table.tBodies[0]?.rows||[]).forEach(row=>{const key=metricKey(row.cells[0]?.textContent||'');if(key)row.cells[0].dataset.i18n=key})})}
function markTranslatableDocuments(){const query=document.querySelector('[data-page="query_pack"]');if(query){const h1=query.querySelector(':scope>h1');if(h1)h1.dataset.i18n='query_document_title';Array.from(query.querySelectorAll(':scope>h3')).forEach(head=>{const key=variantKey(head.textContent);if(key)head.dataset.i18n=key});Array.from(query.querySelectorAll(':scope>p')).forEach(paragraph=>{const text=paragraph.textContent.trim(),span=text.match(/\b(\d{4}\s*[-–—]\s*\d{4}|不限年份|no year limit)\b/i),focus=text.match(/^(?:search context|search focus|检索重点|检索上下文)\s*[:：]\s*(.+)$/i),qa=text.match(/^query\s*qa\s*[:：]\s*(PASS|WARNING|FAIL)/i);if(focus){paragraph.dataset.i18n='search_context';paragraph.dataset.focusKey=searchFocusKey(focus[1])}else if(/^time span|^time policy|^时间范围|^时间策略/i.test(text)&&span){paragraph.dataset.i18n='time_policy';paragraph.dataset.timeSpan=span[1]}else if(qa){paragraph.dataset.i18n='query_qa_summary';paragraph.dataset.status=qa[1].toUpperCase()}else if(span&&(/usage/i.test(text)||/使用/.test(text))&&(/filter/i.test(text)||/筛选/.test(text))){paragraph.dataset.i18n='query_usage';paragraph.dataset.timeSpan=span[1]}})}const candidates=document.querySelector('[data-page="candidate_list"]');if(candidates){const h1=candidates.querySelector(':scope>h1');if(h1)h1.dataset.i18n='candidate_document_title';Array.from(candidates.querySelectorAll(':scope>h2')).forEach(head=>{const text=head.textContent.trim();if(/retrieval.*verification statistics|收割.*核验统计/i.test(text))head.dataset.i18n='retrieval_stats_title';else if(/harvested literature list|candidate literature list|候选文献列表/i.test(text))head.dataset.i18n='harvested_list_title';else if(/dropped examples|excluded examples|剔除示例/i.test(text))head.dataset.i18n='excluded_examples_title'});Array.from(candidates.querySelectorAll(':scope>p')).forEach(paragraph=>{const text=paragraph.textContent.trim(),summary=text.match(/(?:验证通过|verified)\s*[:：]?\s*(\d+).*?(?:待人工|unverified|manual review)\s*[:：]?\s*(\d+).*?(?:剔除|dropped|excluded)\s*[:：]?\s*(\d+)/i);if(/候选清单.*(?:最终|纳入)|candidate records.*(?:final|included)/i.test(text))paragraph.dataset.i18n='candidate_boundary_intro';else if(summary){paragraph.dataset.i18n='candidate_verification_summary';paragraph.dataset.verified=summary[1];paragraph.dataset.pending=summary[2];paragraph.dataset.dropped=summary[3]}});markCandidateStatistics(candidates)}}
...
function candidateColumnClass(value){const text=value.trim().toLowerCase();if(/^(no\.?|id|序号|编号)$/.test(text))return'candidate-col-id';if(/^(title|题名|标题)$/.test(text))return'candidate-col-title';if(/author|作者/.test(text))return'candidate-col-author';if(/^(year|年份|年)$/.test(text))return'candidate-col-year';if(/^doi\b/.test(text))return'candidate-col-doi';if(/verification|验证|核验/.test(text))return'candidate-col-verification';if(/^oa\b|open access|开放获取/.test(text))return'candidate-col-oa';return''}
function candidateTable(article){const tables=Array.from(article?.querySelectorAll('table')||[]);const semantic=tables.find(table=>{const headers=Array.from(table.tHead?.rows[0]?.cells||[]).map(cell=>cell.textContent.trim().toLowerCase());return headers.some(value=>/^(title|题名|标题)$/.test(value))&&headers.some(value=>/^doi\b/.test(value))&&headers.some(value=>/^(year|年份|年)$/.test(value))});return semantic||tables.reduce((best,table)=>(table.tBodies[0]?.rows.length||0)>(best?.tBodies[0]?.rows.length||0)?table:best,null)}
function candidateColumnLayout(){const article=document.querySelector('[data-page="candidate_list"]'),table=candidateTable(article);if(!table)return;table.dataset.candidateTable='true';const headers=Array.from(table.tHead?.rows[0]?.cells||[]);const rows=Array.from(table.tBodies[0]?.rows||[]);headers.forEach((header,index)=>{const columnClass=candidateColumnClass(header.textContent);if(!columnClass)return;header.classList.add(columnClass);rows.forEach(row=>row.cells[index]?.classList.add(columnClass))})}
