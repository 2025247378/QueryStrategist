...
| IEEE Xplore | Advanced Search → Command Search | A/B/C/D1/D2/E、字段重复、`NEAR/10`、`ONEAR/10`、25-term | 待人工填写 |
...
1. 三层参数同时存在时，请求使用三个 `title_and_abstract.search` 过滤器。
2. 排除词没有拼入 OpenAlex API 过滤器，而是在标题和 inverted-index 摘要本地过滤。
3. 输出包含 `verified`、`unverified`、`dropped`、`is_oa`、`oa_status`。
4. `statistics` 各计数与数组长度一致。
